...
public class CameraTestFragment extends TestFragment {
    ...
    protected void initViewEvents(View view) {
        mCapture = (AppCompatImageButton) view.findViewById(R.id.capture);
        mPng = (AppCompatImageView) view.findViewById(R.id.png);
        mPass = (AppCompatButton) view.findViewById(R.id.pass);
        mFail = (AppCompatButton) view.findViewById(R.id.fail);
        mCameralayout = (FrameLayout) view.findViewById(R.id.camera_preview);
        mCameraId = mActivity.getIntent().getIntExtra("camId", 0);
        myWork();
    }

    private void myWork() {
        if (!checkCameraHardware(mActivity)) {
            Toast.makeText(mActivity, "相机不支持", Toast.LENGTH_SHORT)
                    .show();
        } else {
    ...
    public void onPictureTakenThis(byte[] data) {
        ...
        try {
            ...
            e.printStackTrace();
            Toast.makeText(mActivity, "保存照片失败！", Toast.LENGTH_LONG).show();
        }
        finally
        {
            // camera.startPreview();
    ...
    public Camera getCameraInstance(int id) {
        try {
            ...
            mCamera = (Camera) openMethod.invoke(
    ...
    public static void setCameraDisplayOrientation (BaseActivity activity, int cameraId, Camera camera) {
