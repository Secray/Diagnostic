package com.wingtech.diagnostic.util;

/**
 * @author xiekui
 * @date 2017-7-28
 */

public class Constants {
    public static final String BLUETOOTH_STATE_CHANGED = "android.bluetooth.adapter.action.STATE_CHANGED";
    public static final String BLUETOOTH_DISCOVERY_STARTED = "android.bluetooth.adapter.action.DISCOVERY_STARTED";
    public static final String BLUETOOTH_DEVICE_FOUND = "android.bluetooth.device.action.FOUND";
    public static final String BLUETOOTH_DISCOVERY_FINISHED = "android.bluetooth.adapter.action.DISCOVERY_FINISHED";
    public static final String WIFI_STATE_CHANGED = "android.net.wifi.WIFI_STATE_CHANGED";

    public static final int BLUETOOTH_REQUEST_CODE = 0;
    public static final int WIFI_REQUEST_CODE = 1;
    public static final int BATTERY_REQUEST_CODE = 2;
    public static final int E_COMPASS_REQUEST_CODE = 3;
    public static final int GYROSCOPE_REQUEST_CODE = 4;
    public static final int SDCARD_REQUEST_CODE = 5;
    public static final int SIMCARD_REQUEST_CODE = 6;
    public static final int SIM2_REQUEST_CODE = 7;
    public static final int G_SENSOR_REQUEST_CODE = 8;
    public static final int MULTI_TOUCH_REQUEST_CODE = 9;
    public static final int VIBRATOR_REQUEST_CODE = 10;
    public static final int MOUSE_REQUEST_CODE = 11;
    public static final int PROXIMITY_REQUEST_CODE = 12;
    public static final int DISPLAY_REQUEST_CODE = 13;
    public static final int HEADSETKEY_REQUEST_CODE = 14;
    public static final int LIGHTSENSOR_REQUEST_CODE = 15;
    public static final int NFC_REQUEST_CODE = 16;
    public static final int WIRECHARGKEY_REQUEST_CODE = 17;
    public static final int CAMERA_REQUEST_CODE = 18;
    public static final int VGACAMERA_REQUEST_CODE = 19;
    public static final int CAMERAFLASH_REQUEST_CODE = 20;
    public static final int RECIEVER_REQUEST_CODE = 21;
    public static final int HEADSET_REQUEST_CODE = 22;
    public static final int SPEAK_REQUEST_CODE = 23;
    public static final int MIC_REQUEST_CODE = 24;
    public static final int HEADSETMIC_REQUEST_CODE = 25;
    public static final int MODEM_REQUEST_CODE = 26;
    public static final int PROXIMITY_REQUEST_CODE = 11;
    public static final int DISPLAY_REQUEST_CODE = 12;
    public static final int HEADSETKEY_REQUEST_CODE = 13;
    public static final int LIGHTSENSOR_REQUEST_CODE = 14;
    public static final int NFC_REQUEST_CODE = 15;
    public static final int WIRECHARGKEY_REQUEST_CODE = 16;
    public static final int CAMERA_REQUEST_CODE = 17;
    public static final int VGACAMERA_REQUEST_CODE = 18;
    public static final int CAMERAFLASH_REQUEST_CODE = 19;
    public static final int RECIEVER_REQUEST_CODE = 20;
    public static final int HEADSET_REQUEST_CODE = 21;
    public static final int SPEAK_REQUEST_CODE = 22;
    public static final int MIC_REQUEST_CODE = 23;
    public static final int HEADSETMIC_REQUEST_CODE = 24;
    public static final int MOUSE_REQUEST_CODE = 25;



}
